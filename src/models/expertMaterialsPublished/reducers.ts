...
export const expertsSlice = createSlice({
  ...
  reducers: {
    ...
    setFilters: (state, action: PayloadAction<FilterConfigType[]>) => {
      const prevState = state.data.filters.filterConfig;
      const newState = action.payload;
      state.data.filters.filterConfig = prevState.map(
        ({ id, name, checked }) => {
          if (newState.some((val) => val.id === id)) {
            return {
              id,
              name,
              checked: newState?.find((v) => v.id === id)?.checked ?? null,
            };
          }
          return { id, name, checked };
        },
      ...
      const filteredMaterials = allMaterials.filter((material) =>
        state.data.filters.filterConfig.some(({ id, checked }) =>
